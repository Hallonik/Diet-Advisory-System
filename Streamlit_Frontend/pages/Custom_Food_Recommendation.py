...
class Recommendation:
    ...
    def generate(self,):
        ...
        allergic_ingredients = [a.strip().lower() for a in self.allergic_txt.split(';') if a.strip()] if self.allergic_txt else []
        ...
        # Filter out recipes that contain any of the allergic ingredients
        if recommendations is not None:
            filtered_recommendations = []
            for recipe in recommendations:
                recipe_ingredients = [i.lower() for i in recipe['RecipeIngredientParts']]
                if not any(allergen in ingredient for allergen in allergic_ingredients for ingredient in recipe_ingredients):
...
with st.form("recommendation_form"):
    ...
    allergic_ingredients_txt = st.text_input(
        'Specify allergic ingredients to avoid (separated by ";" ):',
        placeholder='e.g., peanuts;shrimp;milk'
    )
    st.caption('Recipes containing any of these ingredients will be excluded.')
