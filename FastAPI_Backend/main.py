...
def output_recommended_recipes(recommendation_dataframe):
    ...
    for _, row in recommendation_dataframe.iterrows():
        ...
        recipe = Recipe(
            Name=row["Name"],
    CookTime=str(row["CookTime"]),
    PrepTime=str(row["PrepTime"]),
    TotalTime=str(row["TotalTime"]),
    RecipeIngredientParts=[
        ingredient.strip().strip('"').replace('c("', '').replace('")', '')
    ...
    RecipeInstructions=[
        instruction.strip().strip('"').replace('c("', '').replace('")', '')
        for instruction in row["RecipeInstructions"].split(" , ")
    ]
        )
...
def update_item(prediction_input: PredictionIn):
    ...
    if output is None:
        logging.info("No recommendation found")
        return {"output": None}
    else:
